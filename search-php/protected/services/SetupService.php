...
class SetupService{
	...
	public static function leadersInfo($area){
		...
		$fans = "select count(fansid) as count from v_fans_area where num>=".$num;
		$tweets = "select sum(statuses_count) as count from t_user u, v_fans_area fa where fa.num>=".$num." and fa.fansid=u.uid";
